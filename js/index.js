function updateResolution() {
	...
	var servNavHeight=(classname == "mobile")?0:$("#serv-navigation").height();
	var servPagePadding={
		w1900: 285, 
		w1300: 165,
		w1000: 125,
	...
	if($("body").hasClass("disabled-onepage-scroll")) { mobileScroll(); }
}

var future_words=["автомобильные дороги", "мосты", "путепроводы", "транспортные развязки", "объекты ПГС", "транспортные системы"];
var future_cur=0;
...
function mobileScroll(event) {
	...
	$("section.page.main").css("clip-path", "inset(" + (scrollTop+logoHeight+15) + "px 0px 0px)");
	$("section.page.main").css("-webkit-clip-path", "inset(" + (scrollTop+logoHeight+15) + "px 0px 0px)");
	
	if($("body").hasClass("disabled-onepage-scroll")) {
		if(scrollTop >= (firstSlideHeight - $("#logo").outerHeight())) {
			$("#logo, #navigation, #hamburger").removeClass("white").addClass("black");
			$("#serv-overlay").addClass("visible");
			pageIndex=2;
		} else {
			$("#logo, #navigation, #hamburger").removeClass("black").addClass("white");
...
$(document).ready(function() {
	...
	$("#serv-navigation a, .main-icons .ico, #navigation-m a, #navigation a[data-index=8]").click(function(){
		if($(window).width() < 800 || $("body").hasClass("disabled-onepage-scroll")) {
			var slide_index=$(this).attr("data-index");
			var slide_offset=$(".page[data-index="+slide_index+"]").offset().top;
			...
			if(typeof(slide_index) != "undefined") {
				menuAction(true);
				$("#serv-navigation a.active").removeClass("active");
				$("#serv-navigation a[data-index='"+slide_index+"']").addClass("active");
				
				$("html, body").animate({ scrollTop: (slide_offset+body_scrolltop-overlay_height) }, 750);
			}
		} else {
			$("#main").moveTo($(this).attr("data-index"));
		}
	});
	$(".main-icons .ico").mouseover(function(){$(this).addClass("active");});
	$(".main-icons .ico").mouseout(function(){$(this).removeClass("active");});
